		...
		$.ajax($.extend(option, {
			...
			error : function(e) {
				...
				if (response && response.fieldErrors && !option.executeExceptionCallback) {
					cntt.error.errorMessage(response.fieldErrors);
					return;
				}
...
cntt.module = (function() {
	...
	function createKCPFormElement() {
		...
		// TODO : 수정 해야 함!!!
		cntt.module.KCP.formParams = {
			pay_method 		: 		"100000000000",
			ordr_idxx 		: 		cntt.module.KCP.orderKCPInfo['goodName'],
			good_name		:		cntt.module.KCP.orderKCPInfo['goodName'],
			good_mny		:		cntt.module.KCP.orderKCPInfo['goodMny'],
			buyr_name		:		cntt.module.KCP.orderKCPInfo['buyrName'],
			buyr_mail		:		"test@test.co.kr",
			buyr_tel1		:		"02-0000-0000",
			buyr_tel2		:		"010-0000-0000",
			req_tx			:		"pay",
			site_cd			:		cntt.module.KCP.properties['g_conf_site_cd'],    
			site_name		:		cntt.module.KCP.properties['g_conf_site_name'],	
			quotaopt		:		"12",
			currency		:		"WON",
			module_type		:		cntt.module.KCP.properties['module_type'],	
			res_cd			:		"",
			res_msg			:		"",
			enc_info		:		"",
			enc_data		:		"",
	...
	function initKCP() {
		...
		cntt.ajax.post('/order/kcpSetRequire', null, function(result) {
			// cntt.payment.KCP.KCPForm 에 저장
			// response setting
			cntt.module.KCP.orderKCPInfo = result.kcpVo;
			cntt.module.KCP.properties = result.KCPProperties;
